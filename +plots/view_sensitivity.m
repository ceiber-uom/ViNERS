...
if isempty(file) || file(1) == '-'
   file = dir(tools.file('eidors~/sens*.mat'));
   if numel(file) == 1, file = [file.folder filesep file.name]; 
   else file = {}; 
   end
end
if isempty(file) || file(1) == '-' || file(1) == '?'
  file = {'sensitivity*.mat'; 'stimulus*.mat'}; 
  if any(named('-af')), file = file([2 1]); end
  [file,folder] = uigetfile(file,[],tools.file('eidors~\'));
...
IS_stimulus = isfield(EM,'v_extracellular');
IS_multimesh = (numel(EM.model) > 1); % multiple meshes in the same file
DO_verbose = ~any(named('-q')); 
...
if any(named('-3d')), basic_3d_plot(EM,electrode_list), return, end
...
%% Generate sensitivity plot (centre of fascicle 1) 
  ...
  if isfield(EM.info,'FascicleTrajectory')    
      ...
      load(tools.INPUT_file(alist,file),'nerve');
...
%% Basic 3D view
function basic_3d_plot(em,chan)

cla
plots.view_mesh(em.model)
...
for ff = 1:nF    
    pot = em.(sprintf('Fascicle%d',ff)).pot(chan,:);
    scatter3(x_(ff),y_(ff),z_(ff),[],pot,'.')
    q98(ff) = quantile(abs(pot),0.98);
end
colormap(tools.magma), caxis([-1 1] * max(q98)), colorbar


return

%% Cross-section of peak value within fascicles 
...
if isempty(xy) && numel(a_list) > 0
  ...
  ax = load(ax_file);
  if isfield(ax,'nerve')
    if isfield(ax.nerve,'fascicles')
         xy_fac = ax.nerve.fascicles(:,:,f_id); % n x 2
    else xy_fac = ax.nerve.outline(:,:,f_id); % n x 
    end
    ...
    try
     ...
     if mean(ok) < 0.9, 
       ...
       warning('axons found in %s did not match outline specified in %s', ...
                strrep(ax_file,tools.file,'~'),ax.nerve.filename)
       xy = []; 
     else
...
if isempty(xy)
  ...
  if nP == 0
    ...
    if isfield(EM.info,'FascicleTrajectory')
      ...
      xyz = tools.from_trajectory(EM,nerve,[gx(:) gy(:)]);
      ok = in_loop([gx(:) gy(:)],nerve.outline([1:end 1],:,ff));
  ...
  for f0 = 1:numel(opts.fascicle)
    ...
    if isempty(xy{ff})
      if isfield(EM.info,'FascicleTrajectory')
        ...
        alist = dir(tools.file('sub~\axons\*.mat'));
        load(tools.INPUT_file(alist,EM.filename),'nerve');
        
        xyf = nerve.outline(:,:,ff);
        outline{ff} = nerve.outline(:,:,ff);
      else      
    ...
    for ii = 1:size(xy{ff},1)
        ...
        if isfield(EM.info,'FascicleTrajectory')
          ...
          xyz = tools.from_trajectory(EM,nerve,xy{ff}(ii,:));
          xyz = permute(xyz(:,[3 2 1],:),[3 2 1]);
          z = cumsum(sqrt(sum(diff(xyz).^2,2))); % length of axon
          z(end+1) = [2 -1]*z([end end-1]); % extend by 1 (because diff)
          z = z - z(find(xyz(:,3) == min(xyz(:,3)),1));
